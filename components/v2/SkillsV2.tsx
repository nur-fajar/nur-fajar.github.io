...
/* ── SKILLS ──────────────────────────────────────────────────────────────
   ...
   dari lib/relatedProjects.ts, indeks yang sama yang dipakai SkillPopup
   halaman root.
...
import { LOGOS, type PathLogo } from '@/content/v2Path';
...
function SkillRow({ name, accent, canHover }: { name: string; accent: Accent; canHover: boolean }) {
  ...
  const proof = useMemo(() => getRelatedProjects(name)[0], [name]);
