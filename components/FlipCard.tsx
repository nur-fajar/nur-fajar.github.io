...
/* ── Playground terminal window: 5 tabs, one flip transition ─────────────
   A filename tab bar (photo.jpg / globe.tsx / sudoku.tsx / chess.tsx /
   quote.md) jumps straight to any page — photo, zodiac globe, sudoku,
   chess, book quotes. Every page stays mounted once it has been shown once
   (Globe always, since it self-animates in the background the same way the
   original vanilla build did; Sudoku/Chess only from their first
   activation, so a visitor who never opens them never pays for puzzle
   generation or the chess engine).

   The switch itself is Framer Motion driving `rotateY` on each page: the
...
const TABS = ['photo.jpg', 'globe.tsx', 'sudoku.tsx', 'chess.tsx', 'quote.md'] as const;
...
export default function FlipCard() {
  ...
  // (Math.random() would pick differently on each) — goTo() re-randomizes
  ...
  return (
    <div className="term-wrap">
      <div className="term-window" id="flip-card">
        <div className="term-header">
          ...
          <div className="term-tabs mono" role="tablist" aria-label="Playground pages">
            {TABS.map((tab, i) => (
              <button
                key={tab}
                type="button"
      ...
      <p className="term-caption mono">{FLIP_TOOLTIPS[current]}</p>
    </div>
  );
}
